...
def login_spotify(user):
    ...
    username = user
    path = os.getcwd()
    sp_oauth.cache_path = path + '/app/support/.cache_' + username

    token_info = sp_oauth.get_cached_token()

    if not token_info:
        auth_url = sp_oauth.get_authorize_url()
        return {'auth': True, 'value': auth_url}
    else:
        token = token_info['access_token']
        return {'auth': False, 'value': token}
...
def findTrackCorr( artist_name, sp, genre):
    ...
    small_df = pd.read_sql("SELECT * FROM small_artist WHERE genre1 in ('"+genre[0]+"', '"+genre[1]+"', '"+genre[2]+"') or genre2 in ('"+genre[0]+"', '"+genre[1]+"', '"+genre[2]+"') or genre3 in ('"+genre[0]+"', '"+genre[1]+"', '"+genre[2]+"')", conn)
